/**
 * Aeitron AI Fine-Tuning Dataset Generator
 * Compiles a comprehensive, high-quality training dataset in standard Chat JSONL format
 * for fine-tuning OpenAI gpt-4o-mini or Open-Source Meta Llama 3 (via Unsloth/LoRA).
 */

const fs = require('fs');
const path = require('path');

const SYSTEM_PROMPT = `You are the executive AI Co-founder & Chief of Staff for Aeitron AI, working directly with Mahmud Hasan (CEO & Founder).
...
const TRAINING_SAMPLES = [
  ...
  // 2. Financials & Profit Margin Strategy
  {
    messages: [
      { role: 'system', content: SYSTEM_PROMPT },
  ...
  // 3. n8n & Technical Automation Architecture
  {
    messages: [
      { role: 'system', content: SYSTEM_PROMPT },
  ...
  // 4. Client Proposals & Closing Deals
  {
    messages: [
      { role: 'system', content: SYSTEM_PROMPT },
  ...
  // 5. Team & Kanban Project Delivery
  {
    messages: [
      { role: 'system', content: SYSTEM_PROMPT },
...
// Write JSONL file
const outputPath = path.join(__dirname, 'aeitron_finetune_dataset.jsonl');
const jsonlContent = TRAINING_SAMPLES.map((sample) => JSON.stringify(sample)).join('\n');
fs.writeFileSync(outputPath, jsonlContent, 'utf-8');

console.log(`✅ Production fine-tuning dataset generated with ${TRAINING_SAMPLES.length} curated conversation pairs.`);
